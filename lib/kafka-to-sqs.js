...
KafkaToSqs.prototype._onError = function (err) {
  this.opts.log.error('KafkaToSqs._onError() :: Kafka stream error:',
    err);
};
...
KafkaToSqs.prototype._createSqsJob = function (kafkaMessage) {
  if (!kafkaMessage.value) {
    this.opts.log.warn('KafkaToSqs._createSqsJob() :: No value',
      'provided, skipping SQS creation. Raw message', kafkaMessage);
    return;
  }
  const message = kafkaMessage.value;

  this.opts.log.info('KafkaToSqs._createSqsJob() :: Received',
    'Tree Search request with jobId:', message.jobId);

  this.sqs.createJob(message)
    .bind(this)
    .catch(function(err) {
      this.opts.log.error('KafkaToSqs._createSqsJob() :: Error:', err);
    });
};
